...
def test_deve_listar_fornecedor_cliente():
    ...
    client.post("/fornecedor-cliente", json={'nome': 'teste1'})
    client.post("/fornecedor-cliente", json={'nome': 'oloko'})
...
def test_deve_pegar_por_id():
    ...
    response = client.post("/fornecedor-cliente", json={'nome': 'teste1'})
    ...
    assert response_get.json()['nome'] == "teste1"
...
def deve_criar_fornecedor_cliente():
    Base.metadata.drop_all(bind=engine)
    Base.metadata.create_all(bind=engine)
    ...
    novo_fornecedor_cliente_copia = novo_fornecedor_cliente.copy()
    novo_fornecedor_cliente_copia['id']=1

    response = client.post("/fornecedor-cliente", json=novo_fornecedor_cliente)
    
    response = client.status_code == 201
    assert response.json()== novo_fornecedor_cliente_copia
...
def test_deve_atualizar_fornecedor_cliente():
    ...
    assert response_put.json()['nome'] == "martinelis"
...
def deve_retornar_nao_encntrado_para_id_nao_existente_na_remocao():
    Base.metadata.drop_all(bind=engine)
    Base.metadata.create_all(bind=engine)
...
def deve_retornar_erro_quando_o_nome_nao_estiver_dentro_dos_limites():
    response = client.post('/fornecedor-cliente', json={
        "nome":"AS"
    })
    ...
    response2 = client.post('/fornecedor-cliente', json={
        "nome": "ABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJSDHJSHABCDEUHJSDSUEJS"
    })
